...
from Archivo import *
#  
j=0;i=0;R=0;ra=0;m=0;n=0;a=0;b=0;vy=0;vyt=0;svy=0;vyp=0;vyct=0;svyc=0;var=0;desv=0;intif=0;intsu=0
...
def vectorx(m,n):
    ...
    vxt=vx.transpose()
    svx=vx.sum()
    vxx  = [i*i for (i,j) in zip(vx,vx)]#Vector "x" al cuadarado.
    vxc=array(vxx)
    vxct=vxc.transpose()
    svxc=vxc.sum()
...
def fprom(m,n):#realiza el promeido de las variables
    global px,py
    svxc,svx,vxct=vectorx(m,n)
    svy,vyct,svyc=vectory(m,n)    
    px=svx/n
    py=svy/n
    """print "py=",py
    print "px=",px"""
...
def fIntConf(m,n):#intervalo de confinaza para la pendiente.
    ...
    ErEiYX0 = [sqrt(CME*((1/n)+(i-px)**2/sxx)) for (i) in zip(vx)]
    ErEiYX=np.array(ErEiYX0,np.float)
    
    ya=array([(i-j) for (i,j) in zip(vy,ErEiYX0)])
    yz=array([(i+j) for (i,j) in zip(vy,ErEiYX0)])
...
def fVarianzaDesvEstandar(m,n):
    ...
    vx, vxt=fvx(m)
    vy, vyt=fvy(m)
    va=array([(i-py)**2 for (i) in zip(vy)])
...
vectorxy(m)
fr(m,n)
***vectorx(m) esta funcion se debe precisar: y diferenciarla de sxy con sxy2
fvx(m)
fvy(m)
vectorx(m)
vectory(m)
